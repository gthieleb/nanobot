...
def main(
    ...
    basedir: str = typer.Option(
        None, "--basedir", help="Base directory for config and data (default: ~/.nanobot)"
    ),
):
    """nanobot - Personal AI Assistant."""
    # Set global base directory if provided
    if basedir:
        from nanobot.config.loader import set_base_dir

        set_base_dir(Path(basedir).expanduser().resolve())
    pass
...
def gateway(
    ...
    langgraph: bool = typer.Option(
        False, "--langgraph", help="Use LangGraph-based agent (experimental)"
    ),
):
    ...
    async def run():
        try:
            if langgraph:
                from nanobot.langgraph.main import main as lg_main

                await lg_main(mode="gateway")
            else:
                await cron.start()
    ...
    if message:
        ...
        if langgraph:
            ...
            async def run_once():
                ...
                await lg_main(mode="agent", message=message)
        ...
        if langgraph:
            ...
            console.print(
                f"{__logo__} LangGraph interactive mode (type [bold]exit[/bold] or [bold]Ctrl+C[/bold] to quit)\n"
            )
            asyncio.run(lg_main(mode="agent"))
        else:
