...
@app.command()
def ralph(
    task: str = typer.Argument(..., help="Task to work on (declarative, what you want)"),
    iterations: int = typer.Option(0, "--iterations", "-i", help="Max iterations (0 = unlimited)"),
    work_dir: str | None = typer.Option(None, "--work-dir", "-w", help="Working directory"),
    model: str | None = typer.Option(None, "--model", "-m", help="Model to use"),
    sandbox: str = typer.Option(
        "none", "--sandbox", "-s", help="Sandbox provider (none, modal, daytona, runloop)"
    ),
    no_stream: bool = typer.Option(False, "--no-stream", help="Disable streaming output"),
):
    """Run agent in autonomous Ralph mode.

    Ralph is an autonomous looping pattern where each iteration starts with
    fresh context. The filesystem and git serve as the agent's memory.

    Example:
        nanobot ralph "Build a REST API" --iterations 5
        nanobot ralph "Create a CLI tool" --sandbox modal
    """
    import os
    from nanobot.config.loader import load_config
    from nanobot.utils.helpers import get_workspace_path
    ...
    config = load_config()
    workspace = Path(work_dir) if work_dir else config.workspace_path
    ...
    iters_label = "unlimited (Ctrl+C to stop)" if iterations == 0 else str(iterations)
    console.print(f"[dim]Iterations: {iters_label}[/dim]")
    console.print(f"[dim]Working directory: {workspace}[/dim]\n")

    try:
        from nanobot.langgraph.deep_agent_helper import run_ralph_mode
        ...
        console.print("[yellow]LangGraph integration not available, using legacy mode[/yellow]")
        _ralph_legacy(task, iterations, workspace, model)


def _ralph_legacy(task: str, iterations: int, workspace: Path, model: str | None) -> None:
    """Fallback Ralph mode using legacy agent loop."""
    from nanobot.config.loader import load_config
    from nanobot.bus.queue import MessageBus
    from nanobot.agent.loop import AgentLoop
    ...
    try:
        while iterations == 0 or iteration <= iterations:
            ...
            iter_display = f"{iteration}/{iterations}" if iterations > 0 else str(iteration)
            prompt = (
            ...
            async def run_iteration():
                return await agent_loop.process_direct(prompt, session_key="ralph")

            response = asyncio.run(run_iteration())
