...
class Config(BaseSettings):
    ...
    def workspace_path(self) -> Path:
        """Get expanded workspace path, respecting --basedir option."""
        from nanobot.config.loader import _BASE_DIR

        workspace_str = self.agents.defaults.workspace
        workspace_path = Path(workspace_str).expanduser()

        # If custom base_dir is set and workspace is relative, prepend it
        if _BASE_DIR and not workspace_path.is_absolute():
            return _BASE_DIR / "workspace"
