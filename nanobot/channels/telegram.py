...
class TelegramChannel(BaseChannel):
    ...
    # Maximum number of polls to track (prevents memory leak)
    MAX_TRACKED_POLLS = 100
    ...
    def _build_keyboard(self, buttons: list) -> InlineKeyboardMarkup | None:
        ...
        # Each button can be: "Label" or ("Label", "callback_data")
        keyboard = []
        for row in buttons:
            row_buttons = []
            for item in row:
                if isinstance(item, tuple):
                    label = item[0]
                    callback_data = item[1] if len(item) > 1 else label
                else:
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if msg.poll and self.config.keyboard_buttons_enabled:
            try:
                ...
                if poll_msg and poll_msg.poll:
                    ...
                    # Prevent memory leak: remove oldest entries if limit reached
                    if len(self._poll_chats) >= self.MAX_TRACKED_POLLS:
                        oldest_key = next(iter(self._poll_chats))
                        del self._poll_chats[oldest_key]
                        logger.debug("Removed oldest poll tracking: {}", oldest_key)
        ...
        if msg.content and msg.content != "[empty message]":
            # Build keyboard once for all chunks
    ...
    async def _on_poll_answer(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # Remove from tracking after answer received
        self._poll_chats.pop(poll_id, None)
